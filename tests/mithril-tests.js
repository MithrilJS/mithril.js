function testMithril(mock) {
	...
	test(function() {
		// Allow scoped attributes via m.extend
		var root = mock.document.createElement("div")
		var success = false

		var mx = m.extend({x:function(){success = true}})

		m.render(root, mx("div", {x:true}))

		return success === true
	})
	test(function() {
		// Allow scoped attributes via component.attrs
		var root = mock.document.createElement("div")
		var success = false
		m.mount(root, {
			attrs:{x:function(){success = true}},
			view:function(m){
				return m("div", {x:true})
			}
		})
	...
	test(function() {
		// component.attrs leaves rest of view signature intact
		var root = mock.document.createElement("div")
		var success = false
		var component = {
			controller:function(){
			...
			attrs:{x:function(node,handler){handler()}},
			view:function(m,ctrl,arg){
				return m("div", {x:function(){
					success = ctrl.instantiated && arg
				}})
			}
		}

		m.mount(root, m.component(component(true)))

		return success === true
	})
	test(function() {
		// Component attrs extensions do not affect external scope
		var root = mock.document.createElement("div")
		var success = false
		m.attrs.x = function(){success=true}
		m.mount(root, {
			view:function(){
				return m("div",{x:true},{
					attrs:{x:function(){success=false}},
					view:function(m){
						return m("div",{x:true})
					}
				})
