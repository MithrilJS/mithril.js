...
function testMithril(mock) {
	...
	test(function() {
		...
		return m("div", {title: "bar"},
			"test0", "test1", "test2", "test3").children[3] === "test3"
	})
	test(function() {
		return m("div", {title: "bar"},
			m("div"), m("i"), m("span")).children[2].tag === "span"
	})
	...
	test(function() {
		...
		return m("div", m("div")).children[0].tag === "div"
	})
	...
	test(function() {
		...
		return root.childNodes[0].childNodes[2]
			.childNodes[0].nodeValue === "Bob"
	})
	test(function() {
		var root = mock.document.createElement("div")
		var redraws = 0
	...
	test(function() {
		...
		var children = root.childNodes[0]
		return children.childNodes[3].childNodes[0].nodeValue === "d" &&
			children.childNodes[4].id === "bar"
	})
	test(function() {
		// https://github.com/lhorie/mithril.js/issues/50
		var root = mock.document.createElement("div")
		...
		var children = root.childNodes[0]
		return children.childNodes[1].childNodes[0].nodeValue === "b" &&
			children.childNodes[2].nodeValue === "test"
	})
	test(function() {
		// https://github.com/lhorie/mithril.js/issues/50
		var root = mock.document.createElement("div")
		m.render(root, m("#foo", [["a", "b"], "test"]))
		return root.childNodes[0].childNodes[1].nodeValue === "b" &&
			root.childNodes[0].childNodes[2].nodeValue === "test"
	})
	test(function() {
		...
		return root.childNodes[0].childNodes[1]
	...
	test(function() {
		...
		var success = false
		m.render(root, m("div", {
			config: function(elem, isInitialized, ctx) { ctx.data = 1 }
		}))
		m.render(root, m("div", {
			config: function(elem, isInitialized, ctx) {
				success = ctx.data === 1
			}
		}))
		return success
	...
	test(function() {
		...
		var index = 0
		var success = true
		var statefulConfig = function(elem, isInitialized, ctx) {
			ctx.data = index++
		}
	...
	test(function() {
		...
		return root.childNodes[0].childNodes
			.map(function(n) {return n.childNodes[0].nodeValue})
			.join("") === "012345"
	})
	test(function() {
		// https://github.com/lhorie/mithril.js/issues/157
	...
	test(function() {
		var args = m.route.parseQueryString(
			"foo=bar&hello%5B%5D=world&hello%5B%5D=mars&hello%5B%5D=pluto")
	...
	test(function() {
		...
		return xhr.$headers["Content-Type"] ===
	...
	test(function(){
		...
		var scriptTag = [].slice.call(
				mock.document.getElementsByTagName("script"))
	...
	test(function(){
		...
		mock.document.removeChild(body)
		return scriptTag.src.indexOf(
				"/test?jsonpCallback=mithril_callback") > -1 &&
			data.foo === "bar1"
	})
	test(function(){
		var body = mock.document.createElement("body")
	...
	test(function() {
		var value
		var deferred = m.deferred()
		deferred.promise.then(function() {return "foo"})
	...
	test(function() {
		var value
		var deferred = m.deferred()
		deferred.promise.then(null, function() {return "foo"})
			.then(function(data) {value = data})
		deferred.reject("test")
		return value === "foo"
	})
	test(function() {
		var value
		var deferred = m.deferred()
		deferred.promise.catch(function() {return "foo"})
			.then(function(data) {value = data})
		deferred.reject("test")
		return value === "foo"
	})
	test(function() {
		var value1, value2
		var deferred = m.deferred()
		deferred.promise.then(function() {throw new Error()})
	...
	test(function() {
		...
		// place. By contrast, a checked exception is defined as anything that
		// is explicitly thrown via the `throw` keyword and that can be
