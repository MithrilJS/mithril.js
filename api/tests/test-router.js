			...
			void (mode === "pushState" ? ["#", "?", "", "#!", "?!", "/foo"] : ["#", "#!"]).forEach(function(prefix) {
				o.spec("router mode: `" + mode + "`, using prefix `" + prefix + "` starting on " + env.protocol + "//" + env.hostname, function() {
					...
					o("throws on invalid `root` DOM node", function() {
						init(prefix) // mandatory for afterEach check
					...
					if (mode === "pushState") o("default route doesn't break back button", function(done) {
					...
					o("onmatch can redirect to another route that has RouteResolver w/ only onmatch", function(done) {
						...
						callAsync(function() {
							callAsync(function() {
								...
								o(root.firstChild.nodeName).equals("DIV")
								o($window.history.state).deepEquals(mode === "pushState" ? {a: 5} : undefined)
					...
					// should equal
					// 2
					if (mode === "pushState" || env.protocol !== "file:") o("onmatch can redirect w/ window.history.back()", function(done) {
						var render = o.spy()
