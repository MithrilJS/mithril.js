...
function initStream(stream) {
	...
	Object.defineProperties(stream, {
			...
			if (!stream._state.endStream) {
				...
				endStream.map(function(value0) {
					if (value0 === true) unregisterStream(stream), unregisterStream(endStream)
					return value0
				})
...
function updateStream(stream, value0) {
	updateState(stream, value0)
	for (var id in stream._state.deps) updateDependency(stream._state.deps[id], false)
	finalize0(stream)
}
function updateState(stream, value0) {
	stream._state.value = value0
...
function updateDependency(stream, mustSync) {
	...
	if (parents.length > 0 && parents.every(active0) && (mustSync || parents.some(changed))) {
		var value0 = stream._state.derive()
		if (value0 === HALT) return false
		updateState(stream, value0)
	}
}
...
m.stream = createStream
var combine0 = m.stream.combine
m.stream.scan = function (reducer, seed, stream0) {
	var newStream = combine0(function (s) {
		return seed = reducer(seed, s._state.value)
	}, [stream0])
	if (newStream._state.state === 0) newStream(seed)
	return newStream
}
var combine1 = _36.combine
m.stream.scanMerge = function(tuples, seed) {
	var streams = tuples.map(function(tuple) {
		var stream1 = tuple[0]
		if (stream1._state.state === 0) stream1(undefined)
		return stream1
	})
	var newStream0 = combine1(function() {
		var changed0 = arguments[arguments.length - 1]
		streams.forEach(function(stream1, idx) {
			if (changed0.indexOf(stream1) > -1) {
				seed = tuples[idx][1](seed, stream1._state.value)
			}
		})
		return seed
	}, streams)
